...
export class CircleEventEffects {
    ...
    getCircleEvent = this.actions$
        .ofType(CircleEventActions.GET_CIRCLE_EVENT)
        .map((action: CircleEventActions.GetCircleEvent) => {
            return action.payload;
        })
        .switchMap((id) => {
            return this.httpClient.get<CircleEvent>(this.baseUrl + 'circleevent/' + id)
                .mergeMap((result) => {
                    var returnValue = [
                        {
    ...
    searchCircleEvent = this.actions$
        .ofType(CircleEventActions.SEARCH_CIRCLE_EVENT)
    ...
    setCircleEventFilters = this.actions$
        .ofType(CircleEventActions.SET_CIRCLE_EVENT_SEARCH_PARAMS)
        .map(() => {
            return {
    ...
    loadNextCircleEventList = this.actions$
        .ofType(CircleEventActions.LOAD_NEXT_CIRCLE_EVENT_LIST)
        .withLatestFrom(this.store$)
    ...
    getCircleEventList = this.actions$
        .ofType(CircleEventActions.GET_CIRCLE_EVENT_LIST)
    ...
    sendEventParticipationRequest = this.actions$
        .ofType(CircleEventActions.SEND_EVENT_PARTICIPATION_REQUEST)
    ...
    getLatestEventParticipations = this.actions$
        .ofType(CircleEventActions.GET_LATEST_EVENT_PARTICIPANTS)
        .map((action: CircleEventActions.GetLatestEventParticipants) => {
            return action.payload;
        })
        .switchMap((payload) => {
            return this.httpClient.get<CircleEventParticipation[]>(this.baseUrl + 'circleeventparticipation/' + payload + '/latest')
                .map((result) => {
                    return {
    ...
    deleteEventParticipation = this.actions$
        .ofType(CircleEventActions.DELETE_EVENT_PARTICIPATION)
    ...
    cancelEventParticipation = this.actions$
        .ofType(CircleEventActions.CANCEL_EVENT_PARTICIPATION)
